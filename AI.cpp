...
Move AI::getBestMove(Board& board, int maxDepth) {
	...
	for (int depth = 2; depth <= maxDepth; depth += 2) {
		...
		// 必勝状態なら早期終了
		if (bestMove.score >= Config::Score::SCORE_WIN - 10000) break;
		
		auto now = std::chrono::steady_clock::now();
		std::chrono::duration<double> elapsed = now - startTime;
		if (elapsed.count() > Config::TIME_LIMIT_SEC * 0.6) break;
	}
...
Move AI::minimaxRoot(Board& board, int depth) {
	...
	if (moves.empty()) return {Config::BOARD_SIZE/2, Config::BOARD_SIZE/2};
	...
	for (auto& m : moves) {
		...
		if (alpha >= Config::Score::SCORE_WIN - 10000) break;
	}
	return bestMove;
}
		...
		for (int x = 0; x < Config::BOARD_SIZE; ++x) {
			...
			for (int dir = 0; dir < 4; ++dir) {
				...
				if (len >= 5) {
					...
					if (openHead && openTail) score += Config::Score::SCORE_OPEN4; // 止められない
					else if (openHead || openTail) score += Config::Score::SCORE_CLOSED4; // 止めないとやばい
				} else if (len == 3) {
					if (openHead && openTail) score += Config::Score::SCORE_OPEN3; // 次にOpen4になるので非常に危険
					else if (openHead || openTail) score += Config::Score::SCORE_CLOSED3;
				} else if (len == 2) {
					if (openHead && openTail) score += Config::Score::SCORE_OPEN2;
				}
			}
		}
	}
